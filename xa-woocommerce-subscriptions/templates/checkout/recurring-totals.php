...
<tr class="recurring-totals">
    <th colspan="2"><?php esc_html_e('Recurring Totals', HF_Subscriptions::TEXT_DOMAIN); ?></th>
</tr>
...
<?php foreach ($recurring_carts as $recurring_cart_key => $recurring_cart) : ?>
    ...
    <tr class="cart-subtotal recurring-total">
        <?php if ($display_th) : $display_th = false; ?>
            <th rowspan="<?php echo esc_attr($carts_with_multiple_payments); ?>"><?php esc_html_e('Subtotal', HF_Subscriptions::TEXT_DOMAIN); ?></th>
            <td data-title="<?php esc_attr_e('Subtotal', HF_Subscriptions::TEXT_DOMAIN); ?>"><?php hf_cart_totals_subtotal_html($recurring_cart); ?></td>
        <?php else : ?>
            <td><?php hf_cart_totals_subtotal_html($recurring_cart); ?></td>
        <?php endif; ?>
    </tr>
<?php endforeach; ?>
<?php $display_th = true; ?>
...
<?php foreach ($recurring_carts as $recurring_cart_key => $recurring_cart) : ?>
    ...
    <?php if ($display_th) : $display_th = false; ?>
            <th rowspan="<?php echo esc_attr($carts_with_multiple_payments); ?>"><?php esc_html_e('Recurring Total', HF_Subscriptions::TEXT_DOMAIN); ?></th>
            <td data-title="<?php esc_attr_e('Recurring Total', HF_Subscriptions::TEXT_DOMAIN); ?>"><?php hf_cart_totals_order_total_html($recurring_cart); ?></td>
    <?php else : ?>

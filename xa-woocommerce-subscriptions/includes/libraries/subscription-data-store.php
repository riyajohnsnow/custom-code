...
class HF_Subscription_Data_Store extends WC_Order_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Order_Data_Store_Interface {
	...
	public function get_orders( $args = array() ) {
		...
		if ( $args['paginate'] ) {
			...
			if ( 'objects' === $args['return'] ) {
				$return = array_map( 'hf_get_subscription', $subscriptions->orders );
			} else {
			...
			if ( 'objects' === $args['return'] ) {
				$return = array_map( 'hf_get_subscription', $subscriptions );
			} else {
	...
	public function save_dates( $subscription ) {
		...
		foreach ( $this->get_props_to_update( $subscription, $date_meta_keys_to_props ) as $meta_key => $prop ) {
			...
			$saved_dates[ $prop ] = hf_get_datetime_from( $subscription->get_time( $prop ) );
		}

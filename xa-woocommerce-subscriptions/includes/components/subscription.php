...
class HF_Subscription extends WC_Order {
    ...
    public function get_type() {
        return 'hf_shop_subscription';
    }

    public function __isset($key) {

        if (!HF_Subscriptions::is_woocommerce_prior_to('3.0') && in_array($key, $this->deprecated_properties)) {
            $is_set = true;
        } else {
    ...
    public function __set($key, $value) {
        ...
        if (in_array($key, $this->deprecated_properties)) {
            ...
            switch ($key) {
                ...
                case 'order' :
                    $function = 'HF_Subscription::set_parent_id( $order_id )';
                    $this->set_parent_id(hf_get_objects_property($value, 'id'));
                    $this->order = $value;
                    break;

                case 'requires_manual_renewal' :
                    $function = 'HF_Subscription::set_requires_manual_renewal()';
                    $this->set_requires_manual_renewal($value);
                    break;

                case 'payment_gateway' :
                    $function = 'HF_Subscription::set_payment_method()';
                    $this->set_payment_method($value);
                    break;

                case 'suspension_count' :
                    $function = 'HF_Subscription::set_suspension_count()';
                    $this->set_suspension_count($value);
                    break;

                default :
                    $function = 'HF_Subscription::update_dates()';
    ...
    public function __get($key) {
        ...
        if (in_array($key, $this->deprecated_properties)) {
            ...
            switch ($key) {
                ...
                case 'order' :
                    $function = 'HF_Subscription::get_parent()';
                    $value = $this->get_parent();
                    break;

                case 'requires_manual_renewal' :
                    $function = 'HF_Subscription::get_requires_manual_renewal()';
                ...
                case 'suspension_count' :
                    $function = 'HF_Subscription::get_suspension_count()';
                    $value = $this->get_suspension_count();
                    break;

                default :
                    $function = 'HF_Subscription::get_date( ' . $key . ' )';
    ...
    public function can_be_updated_to($new_status) {
        ...
        switch ($new_status) {
            ...
            case 'on-hold' :
                ...
                break;
            case 'cancelled' :
                if ($this->payment_method_supports('subscription_cancellation') && ( $this->has_status('pending-cancel') || !$this->has_status(hf_get_subscription_ended_statuses()) )) {
                    $can_be_updated = true;
                } else {
                    $can_be_updated = false;
                }
            ...
            case 'expired' :
                ...
                break;
            case 'trash' :
                if ($this->has_status(hf_get_subscription_ended_statuses()) || $this->can_be_updated_to('cancelled')) {
                    $can_be_updated = true;
                } else {
                    $can_be_updated = false;
                }
    ...
    public function update_status($new_status, $note = '', $manual = false) {
        ...
        if ($new_status !== $old_status || !in_array($old_status_key, array_keys(hf_get_subscription_statuses()))) {

            do_action('hf_subscription_pre_update_status', $old_status, $new_status, $this);

            if (!$this->can_be_updated_to($new_status)) {

                $message = sprintf(__('Unable to change subscription status to "%s".', HF_Subscriptions::TEXT_DOMAIN), $new_status);
            ...
            try {
                ...
                switch ($new_status) {
                    ...
                    case 'pending-cancel' :
                        ...
                        if (0 == $end_date || hf_date_to_time($end_date) < current_time('timestamp', true)) {
                    ...
                    case 'active' :
                        ...
                        hf_make_user_active($this->get_user_id());
                        break;
                ...
                $this->add_order_note(sprintf(__('Unable to change subscription status to "%s". Exception: %s', HF_Subscriptions::TEXT_DOMAIN), $new_status, $e->getMessage()));
                $this->save();
                do_action('hf_subscription_unable_to_update_status', $this, $new_status, $old_status);
                throw $e;
            }
    ...
    protected function status_transition() {
        ...
        if ($this->status_transition) {
            ...
            if (!empty($this->status_transition['from'])) {
                $transition_note = sprintf(__('Status changed from %1$s to %2$s.', HF_Subscriptions::TEXT_DOMAIN), hf_get_subscription_status_name($this->status_transition['from']), hf_get_subscription_status_name($this->status_transition['to']));
                ...
                $transition_note = sprintf(__('Status set to %s.', HF_Subscriptions::TEXT_DOMAIN), hf_get_subscription_status_name($this->status_transition['to']));
            }
    ...
    public function get_valid_statuses() {
        return array_keys(hf_get_subscription_statuses());
    }
    ...
    public function get_completed_payment_count() {
        ...
        if (false === $this->cached_completed_payment_count) {
            ...
            $completed_payment_count = ( ( $parent_order = $this->get_parent() ) && ( null !== hf_get_objects_property($parent_order, 'date_paid') || $parent_order->has_status($this->get_paid_order_statuses()) ) ) ? 1 : 0;
        ...
        $this->cached_completed_payment_count = apply_filters('hf_subscription_payment_completed_count', $completed_payment_count, $this);
    ...
    public function get_total_initial_payment() {
        ...
        return apply_filters('hf_subscription_total_initial_payment', $initial_total, $this);
    }
        ...
        if (empty($date_type)) {
            ...
            switch ($date_type) {
                case 'date_created' :
                    ...
                    $date = is_null($date) ? hf_get_datetime_from(get_the_date('Y-m-d H:i:s', $this->get_id())) : $date;
                    break;
                case 'date_modified' :
    ...
    protected function get_date_prop_key($date_type) {
        $prefixed_date_type = hf_maybe_prefix_key($date_type, 'schedule_');
    ...
    protected function get_related_orders_date($date_type, $order_type = 'all') {
        ...
        if ('last' === $order_type) {
            ...
            $date = (!$last_order ) ? null : hf_get_objects_property($last_order, $date_type);
        } else {
            foreach ($this->get_related_orders('ids', $order_type) as $related_order_id) {
                $related_order = wc_get_order($related_order_id);
                $date = (!$related_order ) ? null : hf_get_objects_property($related_order, $date_type);
    ...
    public function get_date_to_display($date_type = 'next_payment') {
        ...
        if ($timestamp_gmt > 0) {
            ...
            switch ($date_type) {
                case 'end' :
                    $date_to_display = __('Not yet ended', HF_Subscriptions::TEXT_DOMAIN);
                    break;
                case 'cancelled' :
                    $date_to_display = __('Not cancelled', HF_Subscriptions::TEXT_DOMAIN);
                    break;
                case 'next_payment' :
                default :
                    $date_to_display = __('-', HF_Subscriptions::TEXT_DOMAIN);
                    break;
            }
        }
    ...
    public function get_time($date_type, $timezone = 'gmt') {
        ...
        $datetime = hf_date_to_time($datetime);
        return $datetime;
    }
    ...
    public function update_dates($dates, $timezone = 'gmt') {
        ...
        foreach ($dates as $date_type => $datetime) {
            ...
            $utc_timestamp = ( 0 === $datetime ) ? null : hf_date_to_time($datetime);
    ...
    public function delete_date($date_type) {
        ...
        switch ($date_type) {
            case 'date_created' :
                $message = __('The start date of a subscription can not be deleted, only updated.', HF_Subscriptions::TEXT_DOMAIN);
                break;
            case 'last_order_date_created' :
            case 'last_order_date_modified' :
                $message = sprintf(__('The %s date of a subscription can not be deleted. You must delete the order.', HF_Subscriptions::TEXT_DOMAIN), $date_type);
                break;
            default :
                $message = '';
                break;
    ...
    public function can_date_be_updated($date_type) {
        ...
        switch ($date_type) {
            ...
            case 'next_payment' :
            case 'end' :
                if (!$this->has_status(hf_get_subscription_ended_statuses()) && ( $this->has_status('pending') || $this->payment_method_supports('subscription_date_changes') )) {
                    $can_date_be_updated = true;
                } else {
                    $can_date_be_updated = false;
                }
    ...
    protected function calculate_next_payment_date() {
        ...
        $end_time = $this->get_time('end');
            ...
            $next_payment_timestamp = hf_add_time($this->get_billing_interval(), $this->get_billing_period(), $from_timestamp);

            $i = 1;
            while ($next_payment_timestamp < ( current_time('timestamp', true) + 2 * HOUR_IN_SECONDS ) && $i < 3000) {
                $next_payment_timestamp = hf_add_time($this->get_billing_interval(), $this->get_billing_period(), $next_payment_timestamp);
                $i += 1;
            }
    ...
    public function get_formatted_line_subtotal($item, $tax_display = '') {
        ...
        if ($this->is_one_payment()) {
            ...
            $subtotal = hf_price_string($this->get_price_string_details($line_subtotal));
    ...
    public function get_formatted_order_total($tax_display = '', $display_refunded = true) {
        if ($this->get_total() > 0 && '' !== $this->get_billing_period() && !$this->is_one_payment()) {
            $formatted_order_total = hf_price_string($this->get_price_string_details($this->get_total()));
        } else {
    ...
    public function payment_complete($transaction_id = '') {
        ...
        if (0 == $this->get_total_initial_payment() && 1 == $this->get_completed_payment_count() && false != $this->get_parent()) {
            $note = __('Sign-up complete.', HF_Subscriptions::TEXT_DOMAIN);
        } else {
            $note = __('Payment received.', HF_Subscriptions::TEXT_DOMAIN);
        }
    ...
    public function payment_failed($new_status = 'on-hold') {
        ...
        if (false !== $last_order && false === $last_order->has_status('failed')) {
            remove_filter('woocommerce_order_status_changed', 'HF_Subscription_Renewal_Order::maybe_record_subscription_payment');
            $last_order->update_status('failed');
            add_filter('woocommerce_order_status_changed', 'HF_Subscription_Renewal_Order::maybe_record_subscription_payment', 10, 3);
        }

        $this->add_order_note(__('Payment failed.', HF_Subscriptions::TEXT_DOMAIN));
        ...
        if ('cancelled' == $new_status || apply_filters('hf_subscription_max_failed_payments_exceeded', false, $this)) {
            if ($this->can_be_updated_to('cancelled')) {
                $this->update_status('cancelled', __('Subscription Cancelled: maximum number of failed payments reached.', HF_Subscriptions::TEXT_DOMAIN));
            }
    ...
    public function get_related_orders($return_fields = 'ids', $order_type = 'all') {
        ...
        $related_post_ids = HF_Subscriptions::$cache->cache_and_get('hf-related-orders-to-' . $this->get_id(), array($this, 'get_related_orders_query'), array($this->get_id()));
        ...
        return apply_filters('hf_subscription_related_orders', $related_orders, $this, $return_fields, $order_type);
    }
        ...
        foreach ($order_types as $order_type) {
            switch ($order_type) {
                case 'parent':
                    ...
                    break;
                case 'renewal':
                    $related_orders = array_merge($related_orders, HF_Subscriptions::$cache->cache_and_get('hf-related-orders-to-' . $this->get_id(), array($this, 'get_related_orders_query'), array($this->get_id())));
                    break;
                case 'switch':
                    $related_orders = array_merge($related_orders, array_keys(hf_get_switch_orders_for_subscription($this->get_id())));
                    break;
                default:
                    break;
            }
    ...
    public function get_payment_method_to_display() {
        ...
        if ($this->is_manual()) {
            $payment_method_to_display = __('Manual Renewal', HF_Subscriptions::TEXT_DOMAIN);
    ...
    protected function set_payment_method_meta($payment_method_id, $payment_meta) {
        ...
        if (!is_array($payment_meta)) {
            throw new InvalidArgumentException(__('Payment method meta must be an array.', HF_Subscriptions::TEXT_DOMAIN));
        }
    ...
    public function validate_date_updates($dates, $timezone = 'gmt') {
        ...
        if (!is_array($dates)) {
            throw new InvalidArgumentException(__('Invalid format. First parameter needs to be an array.', HF_Subscriptions::TEXT_DOMAIN));
        }

        if (empty($dates)) {
            throw new InvalidArgumentException(__('Invalid data. First parameter was empty when passed to update_dates().', HF_Subscriptions::TEXT_DOMAIN));
        }
        ...
        if (!empty($extra_keys)) {
            throw new InvalidArgumentException(__('Invalid data. First parameter has a date that is not in the registered date types.', HF_Subscriptions::TEXT_DOMAIN));
        }
        ...
        foreach ($this->get_valid_date_types() as $date_type) {
            ...
            if (isset($dates[$date_type])) {
                ...
                if (!empty($datetime) && false === hf_is_datetime_mysql_format($datetime)) {
                    throw new InvalidArgumentException(sprintf(__('Invalid %s date. The date must be of the format: "Y-m-d H:i:s".', HF_Subscriptions::TEXT_DOMAIN), $date_type));
                }
                ...
                if (empty($datetime)) {
                    ...
                    $timestamps[$date_type] = hf_date_to_time($datetime);
                }
            } else {
            ...
            switch ($date_type) {
                case 'end' :
                    if (array_key_exists('cancelled', $timestamps) && $timestamp < $timestamps['cancelled']) {
                        $messages[] = sprintf(__('The %s date must occur after the cancellation date.', HF_Subscriptions::TEXT_DOMAIN), $date_type);
                    }

                case 'cancelled' :
                    if (array_key_exists('last_order_date_created', $timestamps) && $timestamp < $timestamps['last_order_date_created']) {
                        $messages[] = sprintf(__('The %s date must occur after the last payment date.', HF_Subscriptions::TEXT_DOMAIN), $date_type);
                    }
                    if (array_key_exists('next_payment', $timestamps) && $timestamp <= $timestamps['next_payment']) {
                        $messages[] = sprintf(__('The %s date must occur after the next payment date.', HF_Subscriptions::TEXT_DOMAIN), $date_type);
                    }
    ...
    public function add_product($product, $qty = 1, $args = array()) {
        ...
        if ($item_id && $product->backorders_require_notification() && $product->is_on_backorder($qty)) {
            wc_delete_order_item_meta($item_id, apply_filters('woocommerce_backordered_item_meta_name', __('Backordered', HF_Subscriptions::TEXT_DOMAIN)));
        }
    ...
    protected function get_valid_date_types() {
        ...
        if (empty($this->valid_date_types)) {
            $this->valid_date_types = apply_filters('hf_subscription_valid_date_types', array_merge(
                            array_keys(hf_get_subscription_available_date_types()), array(
                'date_created',
                'date_modified',
                'date_paid',
                'date_completed',

...
function hf_cart_totals_shipping_html() {
    ...
    foreach (WC()->cart->recurring_carts as $recurring_cart_key => $recurring_cart) {
        ...
        if (HF_Subscription_Cart::cart_contains_subscriptions_needing_shipping() && 0 !== $recurring_cart->next_payment_date) {
            ...
            foreach ($packages as $i => $base_package) {
                ...
                if (( 1 === count($package['rates']) ) || ( isset($package['rates'][$chosen_initial_method]) && isset($initial_packages[$i]) && $package['rates'] == $initial_packages[$i]['rates'] && apply_filters('hf_cart_totals_shipping_html_price_only', true, $package, $recurring_cart) )) {
                    ...
                    <tr class="shipping recurring-total <?php echo esc_attr($recurring_cart_key); ?>">
                        <th><?php echo esc_html(sprintf(__('Shipping via %s', HF_Subscriptions::TEXT_DOMAIN), $shipping_method->label)); ?></th>
                        <td data-title="<?php echo esc_attr(sprintf(__('Shipping via %s', HF_Subscriptions::TEXT_DOMAIN), $shipping_method->label)); ?>">
                    ...
                    if ($show_package_name) {
                        $package_name = apply_filters('woocommerce_shipping_package_name', sprintf(_n('Shipping', 'Shipping %d', ( $i + 1), HF_Subscriptions::TEXT_DOMAIN), ( $i + 1)), $i, $package);
                    } else {
                        $package_name = '';
                    }
...
function hf_cart_totals_shipping_method_price_label($method, $cart) {
    ...
    if ($method->cost > 0) {
        ...
        $price_label .= __('Free', HF_Subscriptions::TEXT_DOMAIN);
    }

    return $price_label;
}

function hf_cart_totals_taxes_total_html($cart) {
    $value = apply_filters('woocommerce_cart_totals_taxes_total_html', $cart->get_taxes_total());
    echo wp_kses_post(apply_filters('hf_cart_totals_taxes_total_html', hf_cart_price_string($value, $cart), $cart));
}
...
function hf_cart_totals_coupon_html($coupon, $cart) {
    ...
    if (hf_get_coupon_property($coupon, 'enable_free_shipping')) {
        $value[] = __('Free shipping coupon', HF_Subscriptions::TEXT_DOMAIN);
    }
...
function hf_cart_totals_subtotal_html($cart) {
    echo wp_kses_post(hf_cart_price_string($cart->get_cart_subtotal(), $cart));
}
...
function hf_cart_totals_order_total_html($cart) {
    ...
    if (wc_tax_enabled() && $cart->tax_display_cart == 'incl') {
        ...
        if (!empty($tax_string_array)) {
            $value .= '<small class="includes_tax">' . sprintf(__('(Includes %s)', HF_Subscriptions::TEXT_DOMAIN), implode(', ', $tax_string_array)) . '</small>';
        }
    }
...
function hf_cart_price_string($recurring_amount, $cart) {
    ...
    return hf_price_string(apply_filters('woocommerce_cart_subscription_string_details', array(
    ...
    if (isset($cart->$field)) {
        ...
        foreach ($cart->get_cart() as $cart_item) {
            ...
            if (isset($cart_item[$field])) {
                ...
                $value = HF_Subscriptions_Product::get_meta_data($cart_item['data'], $field, $default);
            }
        }
    }
...
function hf_cart_first_renewal_payment_date($order_total_html, $cart) {
    ...
    if (0 !== $cart->next_payment_date) {
        $first_renewal_date = date_i18n(wc_date_format(), hf_date_to_time(get_date_from_gmt($cart->next_payment_date)));
        $order_total_html .= '<div class="first-payment-date"><small>' . sprintf(__('First renewal date: %s', HF_Subscriptions::TEXT_DOMAIN), $first_renewal_date) . '</small></div>';
    }

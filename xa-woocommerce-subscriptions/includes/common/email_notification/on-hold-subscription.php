...
// suspended subscription email - sent to the admin when a subscription is expired.

class HF_Email_On_Hold_Subscription extends WC_Email {

 
	function __construct() {

		$this->id          = 'suspended_subscription';
		$this->title       = __( 'Suspended Subscription', HF_Subscriptions::TEXT_DOMAIN );
		$this->description = __( 'Suspended Subscription emails are sent when a customer manually suspends their subscription.', HF_Subscriptions::TEXT_DOMAIN );

		$this->heading     = __( 'Subscription Suspended', HF_Subscriptions::TEXT_DOMAIN );
		$this->subject     = sprintf( __( '[%s] Subscription Suspended', HF_Subscriptions::TEXT_DOMAIN ), '{blogname}' );
